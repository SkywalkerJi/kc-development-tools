...
const InputCell = ({
  ...
  return (
    <input
      ...
      className="w-12 px-1 py-0.5 border rounded dark:bg-gray-800 dark:border-gray-700 text-sm"
    />
  );
};
...
export default function ProbabilityTable() {
  ...
  return (
    <main className="p-2 lg:p-4 w-full">
      <div className="max-w-[1600px] mx-auto space-y-4 mt-12">
                ...
                {table.getRowModel().rows.map((row) => (
                  <tr key={row.id}>
                    {row.getVisibleCells().map((cell) => (
                      <td
                        key={cell.id}
                        className="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100"
                      >
                        {flexRender(
